 ...
 */
public class JumpRight extends Command {
    public JumpRight(CommandEnum commandEnum) {
        super(commandEnum);
    }
    ...
    public void drawWay(Field field) {
        ...
        robotWay.add(new Point(robotWay.get(robotWay.size() - 1).getX() + Field.SQUARE_SIZE * 2, robotWay.get(robotWay.size() - 1).getY()));
    }

    @Override
    public Point move(Point p) {
        p.setX(p.getX() + Field.SQUARE_SIZE * 2);
        return p;
    }

    @Override
    public JButton createButton(ImageIcon icon) {
        return new JButton("<html>Jump<br>Right</html>");
    }
    ...
    public void drawBlock(Graphics2D g2, Point p, int rectangleWidth) {
        ...
        g2.drawString(getCommandEnum().getName(), x - 20 - rectangleWidth / 3 + rectangleWidth / 3 - metrics.stringWidth(getCommandEnum().getName()) / 2, y + Blocks.FIGURE_HEIGHT / 2 + metrics.getHeight() / 3);
